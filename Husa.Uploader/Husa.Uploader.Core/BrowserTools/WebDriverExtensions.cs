    ...
    public static class WebDriverExtensions
    {
        ...
        public static CoreWebDriver Click(this CoreWebDriver driver, By by, bool isOptional = false, bool isSecondAttemp = false)
        {
            ...
            catch (NoSuchElementException ex)
            {
                if (!isOptional)
                {
                    driver.Logger.LogWarning(ex, "The non-optional element with locator: {by} was not found when processing Request {ResidentialListingRequestId}.", by, driver.UploadInformation.RequestId);
                    throw;
                }
            }
            catch (StaleElementReferenceException ex)
            {
                if (!isSecondAttemp)
                {
                    ...
                    driver.Logger.LogWarning(ex, "Stale reference while trying to click element {by} while processing Request {ResidentialListingRequestId}.", by, driver.UploadInformation.RequestId);
                    return Click(driver, by, isOptional, true);
                }
                driver.Logger.LogError(ex, "Repeated Stale reference while trying to click element {by} while processing Request {ResidentialListingRequestId}.", by, driver.UploadInformation.RequestId);
                throw;
            }
            catch (ElementNotVisibleException ex)
            {
                if (!isOptional)
                {
                    ...
                    driver.Logger.LogError(ex, "Element Not Visible while trying to click element {by} while processing Request {ResidentialListingRequestId}.", by, driver.UploadInformation.RequestId);
                    throw;
                }
            }
            catch (Exception ex)
            {
                ...
                driver.Logger.LogError(ex, "Undetermined exception when trying to click element {by} while processing Request {ResidentialListingRequestId}.", by, driver.UploadInformation.RequestId);
                throw;
            }
        ...
        public static WebDriverWait GetWait(this CoreWebDriver driver, TimeSpan? duration = null)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(duration), duration, "Cannot have a duration of less than 60 seconds");
            }
        ...
        public static CoreWebDriver SubmitForm(this CoreWebDriver driver, By by, bool isOptional = false)
        {
            ...
            catch (NoSuchElementException ex)
            {
                if (!isOptional)
                {
                    driver.Logger.LogWarning(ex, "The non-optional element with locator: {by} was not found when processing Request with {ResidentialListingRequestId}.", by, driver.UploadInformation.RequestId);
                }
            }
        ...
        public static CoreWebDriver WriteTextbox(this CoreWebDriver driver, By by, string value, bool isOptional = false, bool handleAlerts = false, bool doNotClear = false, bool isSecondAttemp = false)
        {
            ...
            if (value == null)
            {
                driver.Logger.LogInformation("Tried to write a null value to textbox with locator: {by} when processing Request with {ResidentialListingRequestId}.", by, driver.UploadInformation.RequestId);
                return driver;
            }
            ...
            if (!driver.UploadInformation.IsNewListing && !doNotClear)
            {
                ...
                {
                    driver.Logger.LogError(ex, "The element with locator: {by} was in an invalid state when processing Request with {ResidentialListingRequestId}.", by, driver.UploadInformation.RequestId);
                    return driver;
                }
            ...
            catch (StaleElementReferenceException ex)
            {
                if (!isSecondAttemp)
                {
                    ...
                    driver.Logger.LogWarning(ex, "Stale reference while trying to write textbox {by} while processing Request {ResidentialListingRequestId}.", by, driver.UploadInformation.RequestId);
                    return WriteTextbox(driver, by, value, isOptional, handleAlerts, doNotClear, true);
                }
                driver.Logger.LogError(ex, "Repeated Stale reference while trying to  write textbox {by} while processing Request {ResidentialListingRequestId}.", by, driver.UploadInformation.RequestId);
                throw;
            }
            if (handleAlerts)
            {
        ...
        public static CoreWebDriver SetMultipleCheckboxById(this CoreWebDriver driver, string id, string values, bool isOptional = false)
        {
            try
            {
                ...
                {
                    //if (!isOptional)
                        driver.Logger.LogWarning("Tried to use a null value in MultiCheckbox with locator: {id} when processing Request with {ResidentialListingRequestId}.", id, driver.UploadInformation.RequestId);
                ...
                bool hasScrollBar = bool.Parse(driver.ExecuteScript("  (function($) { $.fn.hasScrollBar = function() { return this.get(0).scrollHeight > this.height(); }  })(jQuery); return $('#" + id + "').hasScrollBar();  ").ToString());

                foreach (var value in splitValues)
                {
                    try
                    {
                        driver.wait.Until(webDriver => webDriver.FindElement(By.Id(id)).Displayed);  
                        driver.FindElement(By.Id(id));

                        try
                        {
                            int positionOfItem = 0;
                            var posItemY = driver.ExecuteScript(" return jQuery('#" + id + "_" + value + "').position().top;").ToString();
                    ...
                    {
                        driver.Logger.LogError(exception, "Error when processing Request with {ResidentialListingRequestId}.", driver.UploadInformation.RequestId);
                    }
                }
        ...
        public static CoreWebDriver SetSelect(this CoreWebDriver driver, By by, string value, bool isOptional = false)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(value))
                {
                    if (!isOptional)
                    {
                        driver.Logger.LogWarning("Tried to select a null value in Select with locator: {by} when processing Request with {ResidentialListingRequestId}.", by, driver.UploadInformation.RequestId);
                    }
            ...
            catch (NoSuchElementException noSuchElementException)
            {
                if (!isOptional)
                {
                    driver.Logger.LogWarning(noSuchElementException, "Tried to select a non-existing value in Select with locator: {by} when processing Request with {ResidentialListingRequestId}.", by, driver.UploadInformation.RequestId);
                }
            }
            catch (UnexpectedTagNameException unexpectedTagNameException)
            {
                if (!isOptional)
                {
                    driver.Logger.LogWarning(unexpectedTagNameException, "Tried to transform an element with locator: {by} into a Select when processing Request with {ResidentialListingRequestId}.", by, driver.UploadInformation.RequestId);
                }
            }
            ...
            try
            {
                ...
                foreach (var value in splitValues)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        driver.Logger.LogError(ex, "Error when processing Request with {ResidentialListingRequestId}.", driver.UploadInformation.RequestId);
                    }
                }
        ...
        public static CoreWebDriver SetSelect(this CoreWebDriver driver, By by, string value, string fieldLabel, string fieldSection, bool isOptional = false)
        {
            ...
            catch (NoSuchElementException ex)
            {
                if (!isOptional)
                {
                    driver.Logger.LogWarning("Tried to select a non-existing {value} in Select with locator: {by} when processing Request with {ResidentialListingRequestId}.", value, by, driver.UploadInformation.RequestId);
                }
            ...
            catch (UnexpectedTagNameException ex)
            {
                if (!isOptional)
                {
                    driver.Logger.LogWarning("Tried to transform an element with locator: {by} into a Select when processing Request with {ResidentialListingRequestId}.", by, driver.UploadInformation.RequestId);
                }
        ...
        public static CoreWebDriver SetSelectByText(this CoreWebDriver driver, By by, string text, string fieldLabel, string fieldSection, bool isOptional = false)
        {
            ...
            catch (NoSuchElementException ex)
            {
                if (!isOptional)
                {
                    driver.Logger.LogWarning("Tried to select a non-existing value in Select with locator: {by} when processing Request with {ResidentialListingRequestId}.", by, driver.UploadInformation.RequestId);
                }
        ...
        public static CoreWebDriver SwitchTo(this CoreWebDriver driver, string frame, bool switchToParent = false)
        {
            ...
            {
                driver.Logger.LogWarning(ex, "The frame with locator: {frame} was not found when processing Request with {ResidentialListingRequestId}.", frame, driver.UploadInformation.RequestId);
            }
        ...
        public static CoreWebDriver SetRadioButton(this CoreWebDriver driver, By by, object value, bool isOptional = false)
        {
            try
            {
                ...
                var links = driver.FindElements(by).ToList();
